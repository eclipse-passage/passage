...
public abstract class ChainTestsBase {
	...
	protected FakeLicensingRequest certificateRequest() {
		return new FakeLicensingRequest(Collections.emptyMap(), new JsonSerialization<ExaminationCertificate>()
	...
	protected FakeLicensingRequest conditionRequest() {
		return new FakeLicensingRequest(Collections.emptyMap(),
				new JsonSerialization<Condition>().apply(new BaseServiceInvocationResult<>(condition())).data().get());
	}
