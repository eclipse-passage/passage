...
public final class AcquireChainTest extends LbcTestsBase {
	...
	public void acquireable() {
		createLicense(0, 1);
		assertTrue(new Acquire(new JsonLoadedPersistableLicense(() -> root()))
				.apply(new BaseRequestedCertificate(certificate(), requester())).data().get().restrictions().isEmpty());
	}

	@Test
	public void notAcquireable() {
		createLicense(1, 1);
		assertFalse(new Acquire(new JsonLoadedPersistableLicense(() -> root()))
				.apply(new BaseRequestedCertificate(certificate(), requester())).data().get().restrictions().isEmpty());
	}
